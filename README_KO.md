...
**프로젝트 배경:** 학부 4학년 때 개인 프로젝트로 진행했습니다.
...
| 사용 환경 | RAM 구현 |
|---|---|
| ModelSim 시뮬레이션 | 시뮬레이션용 behavioral RAM 모델을 직접 작성 |
| 실제 FPGA 구현 | Vivado에서 제공하는 RAM IP 블록을 영상 처리 RTL에 연결 |

시뮬레이션에서는 직접 작성한 RAM 모델을 사용하고, 실제 FPGA에 올릴 때는 Vivado RAM 블록을 사용했습니다. 개발 과정의 RAM 모델 소스는 [`archive/memory_models/`](archive/memory_models)에 보존했습니다.

## 현재 아카이브로 다시 빌드하려면

제공된 ZIP에는 당시 사용한 Vivado RAM IP 설정이 포함되어 있지 않습니다. 현재 저장소에서 보드 빌드를 재현하려면 `bufferram`과 `line_ram`에 해당하는 RAM IP를 포트·용량·지연·쓰기 모드에 맞게 복원하거나 다시 생성해야 합니다. 재사용한 I²C 의존성과 일부 이전 테스트벤치의 포트·폭도 확인해야 합니다. 이는 현재 보관된 자료를 이용해 재현할 때 필요한 사항입니다.
